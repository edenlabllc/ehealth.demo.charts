...
```yaml
        log_backups: enabled: false/true
``` 
...
```
    ...
    20) For logical replication its need to manually add tables in provider and subscriber this all provided in `PGLOGICAL.md`

    21) For accesing to admin panel its need to create role, user, client and connection in mithril database
...
```
    ...
    - Create clinet_types if they didnt exist in db
```sql
...
```
    ...
    - After creating all objects its need to fill valuse in `fe` charts
    ...
    - More datail about scopes can be found at the [following link](https://edenlab.atlassian.net/wiki/x/v5Ue)
...
```
    ...
    - create role for mis if its dose not exist
```sql
